...
var Rocket = {
  ...
  land: function land() {
    ...
    if (!Rocket.isLanded && !Rocket.isLanding) {
      //land
      landTween = TweenMax.to(Rocket.obj, 2, {
        scale: 0,
        overwrite: 1,
        directionalRotation: Rocket.angle + '_short',
    ...
    } else if (Rocket.isLanded || Rocket.isLanding === true) {
      //take off
      if (landTween) landTween.kill();
      landTween = new TweenMax.to(Rocket.obj, 2, {
        scale: 1,
        overwrite: 1,
        directionalRotation: Rocket.angle + '_short',
        onStart: function onStart() {
  ...
  locate: function locate(left, top) {
  ...
  fly: function fly(timestamp) {
    ...
    if (curve) {
      ...
      //straight line
      coords = {
        x: Rocket.origin.left + xDistance * t,
        y: Rocket.origin.top + yDistance * t
      };
    }

    if (!oldCoords) oldCoords = {
      x: Rocket.origin.left + Rocket.halfX,
      y: Rocket.origin.left + Rocket.halfY
    };
    ...
    if (t !== 0 && curve) {
      ...
      Rocket.obj.css({
        left: coords.x - Rocket.halfX,
        top: coords.y - Rocket.halfY,
    ...
    deathCheck(coords.x, coords.y); //MOVE WRAPPER

    var wrapMoveX = wOffset.left - (coords.x - oldCoords.x);
    var wrapMoveY = wOffset.top - (coords.y - oldCoords.y);
    if (wrapMoveX > 0) wrapMoveX = 0;else if (wrapMoveX < -wrapperW + wrapperW / 3) wrapMoveX = -wrapperW - wrapperW / 3;else if (Rocket.center.x > w / 2 && Rocket.center.x < wrapperW - w / 2) {
      wrapper.css({
        left: wrapMoveX
      });
      wOffset = wrapper.offset();
    }
    if (wrapMoveY > 0) wrapMoveY = 0;else if (wrapMoveY < -wrapperH + wrapperH / 3) wrapMoveX = -wrapperH - wrapperH / 3;else if (Rocket.center.y > h / 2 && Rocket.center.y < wrapperH - h / 2) {
      wrapper.css({
        top: wrapMoveY
      });
      wOffset = wrapper.offset();
    ...
    oldCoords = coords;
    if (progress <= time) anim = window.requestAnimationFrame(Rocket.fly);else {
      window.cancelAnimationFrame(anim);
      Rocket.glow.animate({
        opacity: 0
      }, 500);
      Rocket.leftEngine.animate({
        opacity: 0
      }, 300);
      Rocket.rightEngine.stop().animate({
        opacity: 0
      }, 300);
      Rocket.isMoving = false; //console.log(Rocket.angle);

      $('#waypoint').remove(); //console.log(checkCollision(coords.x,coords.y));

      checkCollision(coords.x, coords.y); //	console.log('Final Angle: '+Rocket.angle);
      //console.log(' ');
    }
  }
};
$(document).ready(function () {
  $('body').mousemove(function (e) {
    ...
    var angle = getAngle(mouseX, Rocket.pos.left + 92.5, mouseY, Rocket.pos.top + 50); // angle =angle - Rocket.angle;
    // if(angle > 180)
    // 	angle -=360;
    // else if(angle<-180)
    // 	angle +=360;
    ...
    $('#debug .angle').html(Math.round(angle)); //start = false;
  });
}); // END READY
...
function flight(x, y, instant) {
  ...
  start = false; //console.log('mouse: '+mouseX+', '+mouseY)
  ...
  if (absAngle > 180) absAngle -= 360;else if (absAngle < -180) absAngle += 360;

  if (absAngle > 20) {
    Rocket.rightEngine.stop().animate({
      opacity: 0.5
    }, 300);
    Rocket.leftEngine.stop().animate({
      opacity: 1
    }, 300);
  } else if (absAngle < -20) {
    Rocket.rightEngine.stop().animate({
  ...
  if (absAngle < 20 && absAngle > -20) {
    ...
    if (Rocket.angle > 90 && endAngle < -90) endAngle += 360;else if (Rocket.angle < -90 && endAngle > 90) endAngle -= 360;
    $({
      angle: Rocket.angle
    }).stop().animate({
      angle: endAngle
    }, {
      duration: cDistance / 125 * 500 / 2,
      step: function step(now, fx) {
        if (fx.pos > 0) Rocket.obj.css({
    ...
    cDistance = curve.length(); //if(cDistance > 400 || Rocket.isMoving){
...
var getPositionData = function getPositionData(el) {
  return $.extend({
...
function adjPath(x, y) {
  ...
  if (absAngle < -90 && absAngle > -150) cp2Angle += 90;
  if (absAngle > 90 && absAngle < 150) cp2Angle -= 90;
  distRatio = Math.abs(absAngle / 90 * 0.5);

  if (absAngle < -150 || absAngle > 150) {
    distRatio = 1.3;
    if (absAngle < 0) cp2Angle += 115;else cp2Angle -= 115;
  }
