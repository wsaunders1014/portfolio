...
import {Bezier} from './bezier-shard.js';
export const Rocket = {
	width:$('#rocket').width(),
	...
	fly: function(timestamp){
		...
		if(curve){
			...
			coords = {x:Rocket.origin.left+(xDistance*t),y:Rocket.origin.top+(yDistance*t)};
		}
		if(!oldCoords)
			oldCoords = {x:Rocket.origin.left+Rocket.halfX,y:Rocket.origin.left+Rocket.halfY};
		if(t !==0 && curve){
			var nextAngle = this.getAngle(coords.x,oldCoords.x,coords.y,oldCoords.y);
	...
	flight:function(x,y,instant){
		...
		absAngle  = this.getAngle(x,Rocket.center.x,y,Rocket.center.y);
...
////// HELPER FUNCTIONS ///////////
export function easeInOut(time, begValue, changeValue, duration) {
...
export function getDistance(x2,x1,y2,y1){
	return	Math.sqrt((Math.pow((x2 - x1),2)) + (Math.pow((y2-y1),2)));
}
Math.radians = function(angle){
	return angle * Math.PI/180;
};
export function getSlope(x2,x1,y2,y1){
	return (y2-y1) / (x2-x1);
}
export function matrix2Angle(matrix){
...
export var getPositionData = function(el) {
    return $.extend({
