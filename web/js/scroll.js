...
$(document).ready(function () {
  $('#scroll-bar').on('mousedown', function (e) {
    ...
    $('body').on('mousemove', function (e) {
      ...
      if (sliderPos < topLimit) sliderPos = topLimit;
      if (sliderPos >= containerH + 15) sliderPos = containerH + 15;
      $('.dragging').offset({
        top: sliderPos
      });
      ...
      if (scrollDist > contentH - containerH + 30) scrollDist = contentH - containerH + 30;
      $('.content div').eq(0).css({
        top: -scrollDist
      });
    }).on('mouseup', function () {
  ...
  $(document).on('wheel mousewheel', function (e) {
    ...
    if (e.originalEvent.wheelDelta < 0 || e.originalEvent.deltaX > 0 || e.originalEvent.deltaY > 0) {
      ...
      slidePercent = sliderTop / (trackH - 30) * contentPercent;
      scrollDist = Math.round(slidePercent * contentH);
      if (scrollDist > contentH - containerH - 0) scrollDist = contentH - containerH - 0;
      $('.content div').eq(0).animate({
        top: -scrollDist
      }, 100); //SCROLL UP
    } else {
  ...
  document.addEventListener('touchmove', function (e) {
    ...
    if (contentsTop + scrollDist < -upperStop) {
      // 0 + -321 = -321 < -3768
      scrollDist = -(upperStop + contentsTop);
    } else if (contentsTop + scrollDist > 0) {
      //0+ -321 > 0
      scrollDist = -contentsTop;
    }

    $('.content').css({
      top: contentsTop + scrollDist
    }); //-321
  }, false);
